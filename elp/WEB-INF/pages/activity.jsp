                ...
                <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="insights-tab" data-bs-toggle="tab" data-bs-target="#insights"
                            type="button" role="tab" aria-controls="insights" aria-selected="false">Insights</button>
                    </li>
                </ul>
                <div class="tab-content" id="myTabContent" style="min-height: 300px; max-height: 370px; overflow-y: auto; -ms-overflow-style: none; scrollbar-width: none;">
                    <div class="tab-pane fade show active" id="history" role="tabpanel" aria-labelledby="history">
                        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-2">
                            <!-- Lecture Card -->
                            <c:forEach var="history" items="${histories}">
                                <div class="col">
                                    <a href="study.do?course=${history.subTopic.topic.course.courseId}" class="text-decoration-none text-dark d-block">
                                        <div class="card border border-light shadow-sm rounded overflow-hidden position-relative transition" style="max-width: 260px; margin: auto;">
                                            <div class="position-relative">
                                                <img src="get_image.do?type=thumbnail&course=${history.subTopic.topic.course.courseId}&img=${history.subTopic.topic.course.thumbnail}" class="w-100 rounded-top" alt="Lecture Thumbnail" style="height: 120px; object-fit: cover;">
                                                <span class="badge bg-dark position-absolute bottom-0 end-0 m-1 px-2 py-1 small">${history.subTopic.formattedTime()}</span>
                                            </div>
                                            <div class="card-body p-2">
                                                <h6 class="fw-semibold mb-1 text-truncate" style="font-size: 0.85rem;">${history.subTopic.title}</h6>
                                                <p class="text-muted small m-0 text-truncate"><i class="fas fa-book me-1"></i>${history.subTopic.topic.course.courseName}</p>
                                                <p class="text-muted small mt-1"><i class="far fa-clock"></i> ${history.getTimeDifference()}</p>
                                            </div>
                                        </div>
                                    </a>
                                </div>
